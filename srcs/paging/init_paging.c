...
void paging_load_directory(uint32 *directory);

static uint32 *current_directory = 0;

struct paging_4gb_chunk *paging_new_4gb(uint8 flags)
{
    uint32 *directory = kzalloc(sizeof(uint32) * PAGING_TOTAL_ENTRIES_PER_TABLE);
    int offset = 0;
    // return NULL;
    for (int i = 0; i < PAGING_TOTAL_ENTRIES_PER_TABLE; i++)
    {
        uint32 *entry = kzalloc(sizeof(uint32) * PAGING_TOTAL_ENTRIES_PER_TABLE);
            for (int b = 0; b < PAGING_TOTAL_ENTRIES_PER_TABLE; b++)
            {
				//this line make weird stuff, that i don't understand yet
...
uint32 *paging_4gb_chunk_get_directory(struct paging_4gb_chunk *chunk)
{
...
{
    return ((uint32)addr % PAGING_PAGE_SIZE) == 0;
}
