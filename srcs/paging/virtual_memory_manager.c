...
uint32 set_page_directory(page_directory *pd)
{
    if (!pd) return FALSE;
...
// Map a page
uint32 map_page(void *phys_address, void *virt_address)
{
    // Get page
    page_directory *pd = current_page_directory;
    ...
    // Get page in table
    pt_entry *page = &table->entries[PT_INDEX((uint32)virt_address)];

    // Map in page
    SET_ATTRIBUTE(page, PTE_PRESENT);
    SET_FRAME(page, (uint32)phys_address);
...
{
    pt_entry *page = get_page((uint32)virt_address);
...
uint32 initialize_virtual_memory_manager(void)
{
    ...
    // Clear page tables
    kmemset(table, 0, sizeof(page_table));
    kmemset(table3G, 0, sizeof(page_table));

    // Identity map 1st 4MB of memory
    for (uint32 i = 0, frame = 0x0, virt = 0x0; i < 1024; i++, frame += PAGE_SIZE, virt += PAGE_SIZE) {
        // Create new page
        pt_entry page = 0;
        SET_ATTRIBUTE(&page, PTE_PRESENT);
    ...
    for (uint32 i = 0, frame = KERNEL_ADDRESS, virt = 0xC0000000; i < 1024; i++, frame += PAGE_SIZE, virt += PAGE_SIZE) {
        // Create new page
        pt_entry page = 0;
        SET_ATTRIBUTE(&page, PTE_PRESENT);
        SET_ATTRIBUTE(&page, PTE_READ_WRITE);
